...
def main() -> None:
    ...
    if not assets:
        print("[WARN] Nenhum ativo configurado; pipeline encerrado sem benchmark.")
        return
    ...
    for asset in assets:
        ...
        dq_args = [PYTHONPATH_EXPORT, PYTHON_EXECUTABLE, "scripts/data/check_data_quality.py", data_path]
        if dq_output:
        ...
        if asset.get("residual_enabled", True):
            ...
            residual_cmd = [
                PYTHONPATH_EXPORT,
                PYTHON_EXECUTABLE,
                "scripts/finance/train_residual_model.py",
                "--metrics",
                metrics_path,
                "--start",
    ...
    benchmark_cmd = [
        PYTHONPATH_EXPORT,
        PYTHON_EXECUTABLE,
        "scripts/bench/benchmark_multi_assets.py",
        "--assets",
        " ".join(assets_args),
        "--years",
