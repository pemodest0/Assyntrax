...
def compute_baseline_errors(returns, dates, H_long, test_start, test_end, train_end):
    test_start = pd.Timestamp(test_start)
    test_end = pd.Timestamp(test_end)
    idx = np.arange(len(returns))
    ...
    mae_p, mae_z, mae_a = [], [], []
    a = ar1_fit(returns[dates <= pd.Timestamp(train_end)])
    for start in starts:
        r0 = returns[start]
        r1 = returns[start + 1]
...
def run_for_ticker(
    ...
    tau, m, k, mean, std = choose_params(
        dates,
        returns,
        tau_grid,
        m_grid,
    ...
    baseline = compute_baseline_errors(returns, dates, H_long, test_start, test_end, train_end)
    if baseline:
