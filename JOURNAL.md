...
- 2026-02-04: Recalibrated thresholds for more STABLE regimes (escape_lo q45, stretch_lo q35, conf_hi q70). Tightened entropy stress gate to require very high entropy plus elevated change rate.
- 2026-02-04: Added timeframe-specific thresholds (daily: escape_lo q50, stretch_lo q40, conf_hi q65; weekly: escape_lo q45, stretch_lo q35, conf_hi q70). Entropy anomaly now uses relative z-score with change-rate condition to avoid global false positives.
- 2026-02-04: Forecast backtest script now accepts `--horizons` and `--models` overrides. Added guard on stability score computation for safety when metrics missing. Lowered daily conf_hi to q60 to increase STABLE share without loosening UNSTABLE criteria.
- 2026-02-04: Added experimental dynamics diagnostics (AMI adaptive tau, Cao E1/E2, Rosenstein LLE/FTLE, RQA DET/LAM/TT, 0-1 chaos test) in `graph_engine/diagnostics.py`. Results are attached per asset as `diagnostics` and included in asset reports; no changes to regime logic.
- 2026-02-04: Added graph entropy metrics (Shannon + Von Neumann) and delta tracking from prior run to governance stress; emits GRAPH_ENTROPY_SHIFT when delta is large. Metrics are also attached to diagnostics.
- 2026-02-04: Added within-run graph entropy shift using recent vs prior label windows (graph_entropy_delta_local) and new GRAPH_ENTROPY_SHIFT_LOCAL stress flag for early warning without a prior run.
- 2026-02-04: Added percentile-based graph entropy shift flag (GRAPH_ENTROPY_SHIFT_P90) computed post-run across the universe and rewrote asset JSONs + audit to reflect the updated stress flags.
- 2026-02-04: Added experimental EWS + CPD diagnostics (rolling AR1/variance/skew/kurt + change-point detection with ruptures fallback) to `graph_engine/diagnostics.py`, surfaced in asset diagnostics and used for new stress flags (EWS_RISING, CPD_RECENT).
- 2026-02-04: Added HDBSCAN/DBSCAN microstate option with optional HMM-style smoothing (Viterbi on label transitions) and CLI flags in `run_graph_regime_universe.py` (micro method/smoothing/params).
- 2026-02-04: Added purge/embargo options to `run_graph_regime_backtest.py` for leakage-safe backtests (train tail purge, test head embargo).
- 2026-02-06: Added sector-aware risk thresholds with group multipliers (daily/weekly) as fallback when ticker-specific thresholds are missing. `run_graph_regime_universe.py` now loads `data/asset_groups.csv`, assigns `group` to assets, and passes it into `get_risk_thresholds`. Weekly MODE_UNSTABLE now also triggers when REGIME_INSTAVEL + LOW_CONFIDENCE. Engine version bumped to `2026.02.06-sector-risk-v1`.
