...
def prepare_financial_series(df: pd.DataFrame, spec: FinancialDatasetSpec) -> Tuple[pd.DataFrame, Dict[str, object]]:
    ...
    if spec.target_type in {"log_return", "simple_return"}:
        if spec.return_col and spec.return_col in out.columns:
            ...
            y_raw = np.log(price).diff() if spec.target_type == "log_return" else price.pct_change()
        else:
    ...
    out = out.assign(y_raw=y_raw).dropna(subset=["y_raw"])
    metadata: Dict[str, object] = {"entity_name": spec.entity_name, "freq": spec.freq, "target_type": spec.target_type}

    if spec.target_type in {"log_return", "simple_return"} and out["y_raw"].abs().max() > 2:
    ...
    out = out.rename(columns={spec.date_col: "date"}).assign(y=out["y_raw"])
...
def compute_confidence_finance(metrics: Dict[str, float], error_std: float, transition_rate: float, novelty: float) -> Dict[str, object]:
    ...
    mase_value = metrics.get("mase", float("nan"))
    dir_acc = metrics.get("dir_acc", float("nan"))
    mase_norm = 1.0 if mase_value != mase_value else float(np.clip(1.5 - mase_value, 0.0, 1.0))
    ...
    add("mase", mase_value, mase_norm, 0.35, "<1 melhor que naive")
    ...
    if score >= 70:
        level, action, status = "HIGH", "OPERAR", "SIM"
    elif score >= 50:
        level, action, status = "MED", "REDUZIR_RISCO", "DEPENDE"
    else:
        level, action, status = "LOW", "NAO_OPERAR", "NAO"

    reasons = []
    if mase_value != mase_value or mase_value >= 1:
...
def plot_master_finance(dates, y_raw, y_pred, naive_pred, out_path, status_eval: Dict[str, object]):
    import matplotlib
    ...
    text = (
        f"status: {status_eval.get('status')}\n"
        f"level: {status_eval.get('level')}\n"
        f"score: {status_eval.get('score')}\n"
        f"action: {status_eval.get('action')}\n"
        f"reasons: {', '.join(status_eval.get('reasons', []))}"
    )
