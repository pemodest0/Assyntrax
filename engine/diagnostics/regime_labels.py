...
from engine.diagnostics.macro_context import (
...
class RegimeClassifier:
    ...
    def run_full_analysis(
        ...
        if write_confidence:
            ...
            status_path = output_path / f"status{filename_suffix}.json"
            _write_status(status_path, confidence)
        ...
        return {
            ...
            "status_json": str(output_path / f"status{filename_suffix}.json") if write_confidence else "",
            "master_plot": master_plot_path,
            "plots": plot_paths,
            "cluster_labels": cluster_labels,
    ...
    def _write_report(
        ...
        status_eval: dict[str, object] | None = None,
        master_plot: str = "",
    ) -> None:
        ...
        if status_eval:
            lines.extend(
                [
                    ...
                    f"- confidence_level: {status_eval.get('level', '')}",
                    f"- score: {status_eval.get('score', '')}",
                    f"- action: {status_eval.get('action', '')}",
                    f"- recommended_horizon: {status_eval.get('recommended_horizon', '')}",
                ]
            )
        if macro_notes:
            lines.extend(
...
def _write_status(path: Path, confidence: dict[str, object]) -> None:
