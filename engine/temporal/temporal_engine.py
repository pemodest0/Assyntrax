...
def evaluate_years(years: Iterable[YearResult], cfg: TemporalConfig) -> TemporalSummary:
    ...
    if len(improvements) < cfg.required_years_min:
        status = "DEPENDE"
        notes.append("Amostra de anos insuficiente para status definitivo.")
    elif win_rate >= cfg.required_win_rate and avg_improvement >= cfg.min_improvement_pct:
        status = "SIM"
    elif win_rate >= 0.4 and avg_improvement > 0:
        status = "DEPENDE"
        notes.append("Ganho positivo, mas inconsistÃªncia entre anos.")
    else:
        status = "NAO"

    if std_improvement > cfg.max_std_improvement:
        notes.append("Variabilidade alta; resultados instÃ¡veis ano a ano.")
...
def select_best_horizon(
    ...
    for horizon, summary in summaries.items():
        if best is None:
            best = summary
            continue
        if summary.status == "SIM" and best.status != "SIM":
            best = summary
            continue
        if summary.status == best.status:
    ...
    if best is None:
        best = TemporalSummary(
            ...
            losses=0,
            gap_to_target=1.0,
            notes=["Nenhum horizonte disponÃ­vel para avaliaÃ§Ã£o."],
        )
...
def build_temporal_report(
    ...
    for name, summary in summaries.items():
        ...
        if summary.status == "SIM" and current.status != "SIM":
            best_model = name
            continue
        if summary.status == current.status:
    ...
    if status == "SIM":
        ...
        notes.append("Previsibilidade parcial: usar apenas em horizontes curtos ou condiÃ§Ãµes estÃ¡veis.")
    else:
        notes.append("NÃ£o hÃ¡ evidÃªncia consistente de previsibilidade Ãºtil.")
