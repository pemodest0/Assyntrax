...
function normalizeRecord(record, riskTruthStatus) {
    const regimeFromState = record.state?.label;
    const signalStatus = String(record.signal_status || "unknown").toLowerCase();
    const fallbackRiskStatus = signalStatus === "validated" || signalStatus === "watch" || signalStatus === "inconclusive" ? signalStatus : "unknown";
    return {
        ...
        confidence: Number(record.confidence ?? record.metrics?.confidence ?? 0),
        quality: Number(record.quality ?? record.metrics?.quality ?? 0),
...
async function GET(request) {
    ...
    if (file) {
        try {
            const data = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$server$2f$data$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["readLatestFile"])(file);
            const m = file.match(/^(.+?)_(daily|weekly)\.json$/i);
            if (m) {
                try {
                    const statusMap = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$server$2f$validated$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["readAssetStatusMap"])();
    ...
    const allowedStatus = statusParam.split(",").map((s)=>s.trim().toLowerCase()).filter(Boolean);
    const allowed = new Set(allowedStatus.length ? allowedStatus : [
        "validated",
        "watch"
    ]);
    const [snap, riskTruth] = await Promise.all([
        (0, __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$server$2f$data$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["readLatestSnapshot"])(),
        (0, __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$server$2f$data$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["readRiskTruthPanel"])()
    ]);
    if (!snap) {
        return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
            error: "no_valid_run"
    ...
    const truthMap = new Map(Array.isArray(riskTruth?.entries) ? riskTruth.entries.map((e)=>[
            String(e.asset_id || ""),
            String(e.risk_truth_status || "unknown")
        ]) : []);
    const records = (Array.isArray(snap.records) ? snap.records : []).map((r)=>normalizeRecord(r, truthMap.get(String(r.asset || "")))).filter((r)=>domain ? r.domain === domain : true).filter((r)=>{
    ...
    if (!records.length) {
        const files = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$server$2f$data$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["listLatestFiles"])();
        return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
            files,
            records: [],
            run_id: snap.runId,
...
//# sourceMappingURL=%5Broot-of-the-server%5D__47a1c0cc._.js.map