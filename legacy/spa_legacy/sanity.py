...
def validate_time_split(
    ...
    if np.any(train_mask & test_mask):
        raise ValueError("Split inválido: sobreposição entre treino e teste.")
    ...
    if test_start is not None:
        if dates[test_mask].min() < test_start:
            raise ValueError("Dados de teste incluem datas antes do início definido.")
    if test_end is not None:
        if dates[test_mask].max() > test_end:
            raise ValueError("Dados de teste incluem datas após o fim definido.")
